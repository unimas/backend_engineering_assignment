...
import pl.neumann.dawid.task.services.*;
...
public class ProductDetailsDefaultService  implements ProductDetailsService {
    ...
    private final TimeLimiter timeLimiter = TimeLimiter.of(TimeLimiterConfig.custom()
            .timeoutDuration(Duration.ofMillis(TIMEOUT_INTERVAL)).build());


    @Override
    public ProductDetailsResponse getProductDetails(int productId, Markets market, Integer customerId) throws Exception {
        ...
        futureProduct = obtainProductInfo(productId);
        try {
    ...
    private PricingService determinePricingHub(Markets marketLanguage) {
        switch (marketLanguage) {
            case POLAND -> {
                return applicationContext.getBean(PricingPolandMockService.class);
            }
            case GERMANY -> {
                return applicationContext.getBean(PricingGermanyMockService.class);
            }
            default -> {
                return applicationContext.getBean(PricingNetherlandsMockService.class);
            }
        }
    }
    ...
    private AvailabilityService determineAvailabilityHub(Markets marketLanguage) {
        switch (marketLanguage) {
            case POLAND -> {
                return applicationContext.getBean(AvailabilityPolandMockService.class);
            }
            case GERMANY -> {
                return applicationContext.getBean(AvailabilityGermanyMockService.class);
            }
            default ->  {
                return applicationContext.getBean(AvailabilityNetherlandsMockService.class);
            }
        }
    }
